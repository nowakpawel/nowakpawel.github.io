...
---
Wielowątkowość pozwala wielu aktywnościom na równoległe wykonywanie swoich zadań. 
Wielowątkowość możemy podzielić na dwie kategorie:
- Process-based multitasking - Pozwala na równoległe włączanie różnych procesów (odtwarzanie muzyki z Winampa, w czasie, gdy edytujemy tekst w MS Word).
- Thread-based multitasking -  Pozwala komponentom tego samego programu na równoległe wykonywanie różnych czynności (Drukowanie oraz edycja dokumentu w tym samym czasie).

# Wątek vs Proces
- Dwa wątki współdzielą ten sam adres w pamięci.
...
- Wątek to niezależna część jednego procesu (programu), która może zostać wykonana w separacji od innej części - tak jak w przykładzie w MS Word.
- Wiele wątków w programie może być wykonywanych niezależnie od siebie.
- W czasie wykonywania wątki programu istnieją we wspólnej przestrzeni adresowej pamięci komputera. Dzięki temu mogą współdzielić dane i instrukcje (co sprawia również problemy, ale o tym za chwilę).
...
```
Aby odpalić wątek, tworzymy obiekt naszej nowo utworzonej klasy i wywołujemy metodę `start()`. Dlaczego `start()`, a nie nadpisaną wcześniej `run()`? Otóż sama odpalenie metody `run()` zadziała, ale nie wykona instrukcji w nowym wątku.
...
## Implementujemy interfejs funkcjonalny `Runnable`
Innym sposobem na stworzenie nowego wątku jest zaimplementowanie metody `run()` interface'u `Runnable`. Ponieważ jest to interface funkcjonalny, możemy go zaimplementować poprzez Lambdę.
```java
...
W przeważającej większości przypadków najlepiej jest wybrać implementację interface'u. Powód jest bardzo prosty. W Javie możemy dziedziczyć tylko po jednej klasie (W odróżnieniu od, np. C++). Zatem, jeśli stworzylibyśmy wątek bezpośrednio od klasy `Thread`, stracilibyśmy możliwość dziedziczenia dla tego konkretnego wątku. Tego sposobu używa się tylko w szczególnych przypadkach.
...
Po odpaleniu aplikacji spodziewamy się, że wartośc pola counter będzie wynosiła 20. - Jeden wątek zwiększy wartość pola counter do 10, a drugi zacznie liczyć od 10 i zwiększy do 20. Prosta sprawa... Otóż nie.
Kiedy odpalimy program kilka razy zobaczymy, że wartość `counter` nie zawsze dochodzi do wartości do 20. (W jednym z przypadków, kiedy testowałem to rozwiązanie, counter zatrzymał się na 16\*).
...
```
Zastosowanie `synchronized` w metodzie powoduje dwie rzeczy:
...
Problem można częściowo rozwiązać za pomocą `volatile`, jednak w odróżnieniu od `synchronized`, gwarantuje to tylko , że wartość zapisana z/do pamięci będzie od razu widoczna dla innych wątków, bez uwzględnienia atomowości (ang. atomicity).
